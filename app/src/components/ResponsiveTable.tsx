...
export function ResponsiveTable<T>({ data, columns, onRowClick }: ResponsiveTableProps<T>) {
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="hidden md:block overflow-x-auto border rounded-xl shadow-sm bg-white dark:bg-gray-800 dark:border-gray-700">
            ...
            <div className="md:hidden space-y-3">
                {sortedData.map((item, i) => (
                    <div
                        key={i}
                        onClick={() => onRowClick?.(item)}
                        className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border dark:border-gray-700 active:scale-[0.98] transition-transform"
                    >
                        ...
                        <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm">
                            {columns.slice(1).map((col, j) => (
                                <div key={j} className="flex flex-col">
                                    <span className="text-xs text-gray-500 dark:text-gray-400 uppercase">{col.mobileLabel || col.header}</span>
